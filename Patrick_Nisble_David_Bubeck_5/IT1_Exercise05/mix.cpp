...
std::string mix(std::string stringInput) {
    ...
    //int positionLine = 0; //position for "-"
    for(int i = 0; i < stringInput.length(); i++) {
    ...
    if(positionLine != std::string::npos) {
        ...
        int shuffleEndLeftString = substringLeft.length() - positionLine;
        int shuffleStartRightString = 2; //we want to hold the position for "-" and the first letter
        int shuffleEndRightString = substringRight.size() - positionLastLetter;
 ...
 * for every entry separately and put them after the shuffle into a new string.
 */
